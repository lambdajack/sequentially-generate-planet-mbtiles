	...
	for _, file := range f {
		if !file.IsDir() {
			...
			if _, err := os.Stat(filepath.FromSlash(folders.MbtilesFolder + "/" + outFileName)); os.IsNotExist(err) {
				err := generatembtiles.GenerateMbTiles(file.Name(), outFileName, folders.PbfSlicesFolder, folders.MbtilesFolder, folders.CoastlineFolder, buildthirdpartycontainers.ContainerTilemakerName, folders.TilemakerConfigFile, folders.TilemakerProcessFile)
				if err != nil {
